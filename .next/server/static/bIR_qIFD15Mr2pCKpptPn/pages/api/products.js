...
/* harmony default export */ __webpack_exports__["default"] = (async (req, res) => {
  ...
  let products = [];
  const totalDocs = await _models_Product__WEBPACK_IMPORTED_MODULE_0__[/* default */ "a"].countDocuments();
  const totalPages = Math.ceil(totalDocs / pageSize);

  if (pageNum === 1) {
    products = await _models_Product__WEBPACK_IMPORTED_MODULE_0__[/* default */ "a"].find().limit(pageSize);
  } else {
    const skips = pageSize * (pageNum - 1);
    products = await _models_Product__WEBPACK_IMPORTED_MODULE_0__[/* default */ "a"].find().skip(skips).limit(pageSize);
...
async function connectDb() {
  ...
  const db = await mongoose__WEBPACK_IMPORTED_MODULE_0___default.a.connect("<insert-mongo-srv>", {
    useCreateIndex: true,
